...
def test_model(model, tokenizer, test_data_path, device, fold_num, batch_size=16, max_length=128):
    ...
    with open(f'statistics/results_siku_fold_{fold_num + 1}.json', 'w', encoding='utf-8') as f:
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(data_items)):
        ...
        model.save_pretrained(f'model_siku_kfold/verse_prose_model_fold_{fold + 1}')
        tokenizer.save_pretrained(f'model_siku_kfold/verse_prose_model_fold_{fold + 1}')
    ...
    with open('statistics/siku_kfold_metrics.json', 'w') as f:
        json.dump({
