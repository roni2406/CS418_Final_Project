...
def test_model(model, tokenizer, test_data_path, device, batch_size=16, max_length=128):
    ...
    with open('statistics/results_guwen.json', 'w', encoding='utf-8') as f:
...
def count_parameters(model):
    ...
    # Optional: Print parameter count by layer
    print('\nParameters by layer:')
...
def main():
    ...
    model.save_pretrained('model_guwen/verse_prose_model')
    tokenizer.save_pretrained('model_guwen/verse_prose_model')

    # # Load the saved model and tokenizer, use for inference results if don't want to train
    # model = AutoModelForSequenceClassification.from_pretrained('model_guwen/verse_prose_model')
    # tokenizer = AutoTokenizer.from_pretrained('model_guwen/verse_prose_model')
    # model.to(device)
    
    print("Model and tokenizer saved to 'model_guwen/verse_prose_model'")
